...
export function ProductImportModal({ onClose, onSaved }: Props) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [parsing, setParsing] = useState(false);
  const [parseError, setParseError] = useState("");
  const [preview, setPreview] = useState<
    ImportPreview<ProductImportRowData> | null
  >(null);
  ...
  async function handleFile(file: File) {
    ...
    try {
      const parsed = await productImportService.parse(file);
      setPreview(parsed);
    } catch (err) {
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await productImportService.confirm(rows);
      setResult(res);
      onSaved();
    } catch (err) {
      setConfirmError(
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    Código já cadastrado atualiza o produto; código novo
                    cria. Categoria e marca são criadas automaticamente se
                    não existirem.
                  </p>
                </div>
            ...
            {preview && (
              <>
                <div className="flex flex-wrap gap-3 text-sm">
                  ...
                  <span className="rounded-lg bg-[var(--info-soft,var(--surface-hover))] px-3 py-1.5 font-medium text-[var(--text-primary)]">
                    ...
                    <tbody>
                      {preview.rows.map((row) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2">{row.data.code}</td>
                          <td className="px-3 py-2">
