...
import {
  ...
  type PartnerImportRowData,
} from "@/services/partner-import.service";
...
export function PartnerImportModal({ onClose, onSaved }: Props) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [parsing, setParsing] = useState(false);
  const [parseError, setParseError] = useState("");
  const [preview, setPreview] = useState<
    ImportPreview<PartnerImportRowData> | null
  >(null);
  ...
  async function handleFile(file: File) {
    ...
    try {
      const parsed = await partnerImportService.parse(file);
      setPreview(parsed);
    } catch (err) {
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await partnerImportService.confirm(rows);
      setResult(res);
      onSaved();
    } catch (err) {
      setConfirmError(
  ...
  function downloadErrorReport() {
    ...
    void saveExportFile(
      ...
      "erros-importacao-parceiros.csv",
      "text/csv"
    );
  }
      ...
      <div className="my-8 w-full max-w-5xl rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-lg">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-lg font-bold text-[var(--text-primary)]">
            Importar parceiros via planilha
          </h2>
        ...
        {result ? (
          <div className="space-y-5">
            <div className="flex items-center gap-3 rounded-xl border border-[var(--success)] bg-[var(--success-soft)] p-4 text-sm text-[var(--success)]">
              ...
              Importação concluída: {result.created} parceiro(s)
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-[var(--text-primary)]">
                    Planilha de parceiros (.xlsx ou .csv)
                  </p>
                  <p className="text-xs text-[var(--text-muted)]">
                    CPF/CNPJ já cadastrado atualiza o parceiro; documento
                    novo cria. Papéis aceitos: CLIENTE, FORNECEDOR,
                    TRANSPORTADORA, REPRESENTANTE (separados por vírgula).
                  </p>
                </div>
                    ...
                    <tbody>
                      {preview.rows.map((row) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2">{row.data.document}</td>
                          <td className="px-3 py-2">
                            {row.data.legalName}
                          </td>
