...
import {
  ppeDeliveryPublicService,
  type PpeDeliveryPublicInfo,
...
function ConfirmarEpiContent() {
  const searchParams = useSearchParams();
  const id = searchParams.get("id") ?? "";
  const token = searchParams.get("token") ?? "";

  const [info, setInfo] = useState<PpeDeliveryPublicInfo | null>(
    null
  );
  ...
  async function handleConfirm() {
    ...
    try {
      await ppeDeliveryPublicService.confirm(id, token);
        ...
        {loading ? (
          ...
          <div className="mt-4 space-y-2">
            ...
            <p className="text-sm text-[var(--text-muted)]">
              Obrigado, {info?.employeeName}. Já pode fechar
              esta página.
            </p>
          </div>
        ) : (
          info && (
            <div className="mt-4 space-y-4">
              <p className="text-sm text-[var(--text-muted)]">
                Confirme que você recebeu o EPI abaixo, entregue
              ...
              <div className="space-y-1 rounded-xl border border-[var(--border)] p-4 text-sm">
                ...
                <p>
                  <strong>EPI:</strong> {info.ppeTypeName}
                </p>
                <p>
...
export default function ConfirmarEpiPage() {
  return (
