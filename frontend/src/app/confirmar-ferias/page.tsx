...
import {
  vacationConfirmationPublicService,
  type VacationPublicInfo,
...
function ConfirmarFeriasContent() {
  const searchParams = useSearchParams();
  const id = searchParams.get("id") ?? "";
  const token = searchParams.get("token") ?? "";

  const [info, setInfo] = useState<VacationPublicInfo | null>(null);
  ...
  async function handleConfirm() {
    ...
    try {
      await vacationConfirmationPublicService.confirm(id, token);
    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--background)] p-8">
      <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-sm">
        ...
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">
          Confirmação de recebimento de férias
        </h1>
        ...
        {loading ? (
          ...
          info && (
            <div className="mt-4 space-y-4">
              <p className="text-sm text-[var(--text-muted)]">
                Confirme que você recebeu o recibo de férias abaixo, gerado
              ...
              <div className="space-y-1 rounded-xl border border-[var(--border)] p-4 text-sm">
                ...
                <p>
                  <strong>Período:</strong> {date(info.startDate)} a{" "}
                  {date(info.endDate)}
                </p>
                <p>
                  <strong>Retorno:</strong> {date(info.returnDate)}
                </p>
                <p>
                  <strong>Dias:</strong> {info.days}
                </p>
                <p>
...
export default function ConfirmarFeriasPage() {
  return (
    <Suspense fallback={null}>
      <ConfirmarFeriasContent />
    </Suspense>
  );
}
