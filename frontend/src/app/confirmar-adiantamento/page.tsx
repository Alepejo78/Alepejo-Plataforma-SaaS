...
import {
  salaryAdvanceConfirmationPublicService,
  type SalaryAdvancePublicInfo,
...
function ConfirmarAdiantamentoContent() {
  const searchParams = useSearchParams();
  const id = searchParams.get("id") ?? "";
  const token = searchParams.get("token") ?? "";

  const [info, setInfo] = useState<SalaryAdvancePublicInfo | null>(null);
  ...
  async function handleConfirm() {
    ...
    try {
      await salaryAdvanceConfirmationPublicService.confirm(id, token);
    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--background)] p-8">
      <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-sm">
        ...
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">
          Confirmação de recebimento de adiantamento
        </h1>
        ...
        {loading ? (
          ...
          info && (
            <div className="mt-4 space-y-4">
              <p className="text-sm text-[var(--text-muted)]">
                Confirme que você recebeu o adiantamento salarial abaixo,
              ...
              <div className="space-y-1 rounded-xl border border-[var(--border)] p-4 text-sm">
                ...
                <p>
                  <strong>Solicitado em:</strong> {date(info.requestDate)}
                </p>
                <p>
                  <strong>Valor:</strong> {money(info.amount)}
                </p>
                <p>
                  <strong>Parcelas previstas p/ desconto:</strong>{" "}
                  {info.installments}
                </p>
              </div>
...
export default function ConfirmarAdiantamentoPage() {
  return (
    <Suspense fallback={null}>
      <ConfirmarAdiantamentoContent />
    </Suspense>
  );
}
