...
import {
  thirteenthConfirmationPublicService,
  type ThirteenthPublicInfo,
...
function ConfirmarDecimoTerceiroContent() {
  const searchParams = useSearchParams();
  const id = searchParams.get("id") ?? "";
  const itemId = searchParams.get("itemId") ?? "";
  const token = searchParams.get("token") ?? "";

  const [info, setInfo] = useState<ThirteenthPublicInfo | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [confirming, setConfirming] = useState(false);
  const [done, setDone] = useState(false);

  const linkInvalid = !id || !itemId || !token;
  ...
  async function handleConfirm() {
    ...
    try {
      await thirteenthConfirmationPublicService.confirm(id, itemId, token);
    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--background)] p-8">
      <div className="w-full max-w-md rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-sm">
        ...
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">
          Confirmação de recebimento de 13º salário
        </h1>
        ...
        {loading ? (
          ...
          info && (
            <div className="mt-4 space-y-4">
              <p className="text-sm text-[var(--text-muted)]">
                Confirme que você recebeu o recibo de 13º salário abaixo,
                gerado por <strong>{info.companyName}</strong>:
              </p>

              <div className="space-y-1 rounded-xl border border-[var(--border)] p-4 text-sm">
                <p>
                  <strong>Colaborador:</strong> {info.employeeName}
                </p>
                <p>
                  <strong>Ano:</strong> {info.year}
                </p>
                <p>
                  <strong>Parcela:</strong> {info.installment}ª
                </p>
                <p>
...
export default function ConfirmarDecimoTerceiroPage() {
  return (
    <Suspense fallback={null}>
      <ConfirmarDecimoTerceiroContent />
    </Suspense>
  );
}
